...
from pcdet.ops.sparse_block import SparseBasicBlock, make_sparse_convmodule
from pcdet.ops import spconv as spconv
#from pcdet.models import BACKBONES
...
class SparseEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.order = self.model_cfg.ORDER
        self.base_channels = base_channels
        self.output_channels = self.model_cfg.OUTPUT_CHANNELS
        self.encoder_channels = self.model_cfg.ENCODER_CHANNELS
        self.encoder_paddings = self.model_cfg.ENCODER_PADDINGS
        self.stage_num = len(self.encoder_channels)
        self.fp16_enabled = False
        # Spconv init all weight on its own

        assert isinstance(self.order, (list, tuple)) and len(self.order) == 3
