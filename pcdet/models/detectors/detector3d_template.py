...
from pcdet.models import backbones_2d, backbones_3d, backbones_cam, necks, vtransforms, backbones_lidar, fusers, dense_heads, roi_heads
from pcdet.models.backbones_2d import map_to_bev
from pcdet.models.backbones_3d import pfe, vfe
#from ..backbones_cam import swin
...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = [
            'backbone_cam', 'neck', 'vtransform', 'backbone_3d', 'vfe', 'backbone_lidar', 
            'fuser', 'backbone_2d', 'neck1', 'dense_head', 'map_to_bev_module', 'pfe', 'point_head', 'roi_head'
        ]
    ...
    def build_backbone_cam(self, model_info_dict):
        if self.model_cfg.get('BACKBONE_CAM', None) is None:
            return None, model_info_dict

        backbone_cam_module = backbones_cam.__all__[self.model_cfg.BACKBONE_CAM.NAME](
            model_cfg=self.model_cfg.BACKBONE_CAM
        )
        #model_info_dict['num_point_features'] = backbones_cam_module.get_output_feature_dim()
        model_info_dict['module_list'].append(backbone_cam_module)
        return backbone_cam_module, model_info_dict
    
    def build_neck(self, model_info_dict):
        if self.model_cfg.get('NECK', None) is None:
            return None, model_info_dict

        necks_module = necks.__all__[self.model_cfg.NECK.NAME](
            model_cfg=self.model_cfg.NECK
        )
        #model_info_dict['num_point_features'] = necks_module.get_output_feature_dim()
        model_info_dict['module_list'].append(necks_module)
        return necks_module, model_info_dict
    
    def build_neck1(self, model_info_dict):
        if self.model_cfg.get('NECK1', None) is None:
            return None, model_info_dict

        necks_module = necks.__all__[self.model_cfg.NECK1.NAME](
            model_cfg=self.model_cfg.NECK1
        )
        #model_info_dict['num_point_features'] = necks_module.get_output_feature_dim()
        model_info_dict['module_list'].append(necks_module)
        return necks_module, model_info_dict
    ...
    def build_vtransform(self, model_info_dict):
        ...
        vtransforms_module = vtransforms.__all__[self.model_cfg.VTRANSFORM.NAME](
            model_cfg=self.model_cfg.VTRANSFORM
        )
        #model_info_dict['num_point_features'] = vtransforms_module.get_output_feature_dim()
        model_info_dict['module_list'].append(vtransforms_module)
        return vtransforms_module, model_info_dict
    
    def build_backbone_lidar(self, model_info_dict):
        if self.model_cfg.get('BACKBONE_LIDAR', None) is None:
            return None, model_info_dict

        backbone_lidar_module = backbones_lidar.__all__[self.model_cfg.BACKBONE_LIDAR.NAME](
    ...
    def build_fuser(self, model_info_dict):
        ...
        fuser_module = fusers.__all__[self.model_cfg.FUSER.NAME](
