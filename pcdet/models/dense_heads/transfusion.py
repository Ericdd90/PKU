...
#from pcdet.models.builder import build_loss
from pcdet.models.model_utils.transformer import FFN, PositionEmbeddingLearned, TransformerDecoderLayer
...
class TransFusionHead(nn.Module):
    def __init__(
        ...
        self.auxiliary = self.model_cfg.AUXILIARY
        ...
        self.ffn_channel=self.model_cfg.FFN_CHANNEL
        self.dropout=self.model_cfg.DROPOUT
        self.common_heads=self.model_cfg.COMMON_HEADS
        
        self.use_sigmoid_cls = self.model_cfg.LOSS_CLS.get("use_sigmoid", False)
        if not self.use_sigmoid_cls:
            self.num_classes += 1
        self.loss_cls = self.model_cfg.LOSS_CLS
        self.loss_bbox = self.model_cfg.LOSS_BBOX
        #self.loss_iou = self.model_cfg.LOSS_IOU
        self.loss_heatmap = self.model_cfg.LOSS_HEATMAP
        self.bbox_coder = self.model_cfg.BBOX_CODER
        ...
        self.class_encoding = nn.Conv1d(self.num_classes, self.hidden_channel, 1)
        ...
        for i in range(self.num_decoder_layers):
            self.decoder.append(
                TransformerDecoderLayer(
                    ...
                    self.dropout,
                    self.activation,
                    self_posembed=PositionEmbeddingLearned(2, self.hidden_channel),
                    cross_posembed=PositionEmbeddingLearned(2, self.hidden_channel),
                )
            )
