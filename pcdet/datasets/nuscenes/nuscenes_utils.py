...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        for cam in camera_types:
            ref_cam_token = sample["data"][cam]
            ref_cam_path, _, ref_camera_intrinsics = nusc.get_sample_data(ref_cam_token)
            cam_info = obtain_sensor2top(
                nusc, ref_cam_token, l2e_t, l2e_r_mat, e2g_t, e2g_r_mat, cam
            )
            cam_info.update(camera_intrinsics=ref_camera_intrinsics)
