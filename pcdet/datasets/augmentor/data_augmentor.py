...
class DataAugmentor(object):
    ...
    def ImageAug3D(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.ImageAug3D, config=config)
        
        final_dim=config['FINAL_DIM']
        resize_lim=config['RESIZE_LIM']
        bot_pct_lim=config['BOT_PCT_LIM']
        rot_lim=config['ROT_LIM']
        rand_flip=['RAND_FLIP']
        ...
        transforms = []
        for img in imgs:
            resize, resize_dims, crop, flip, rotate = augmentor_utils.sample_augmentation(data)
            post_rot = torch.eye(2)
            post_tran = torch.zeros(2)
            new_img, rotation, translation = augmentor_utils.img_transform(
                img,
                post_rot,
                post_tran,
                resize=resize,
    ...
    def ImageNormalize(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.ImageNormalize, config=config)
        mean = config['MEAN']
        std = config['STD']
        ...
        data_dict["img"] = [self.compose(img) for img in data_dict["img"]]
        data_dict["img_norm_cfg"] = dict(mean=mean, std=std)
        return data_dict
    
    def GridMask(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.GridMask, config=config)
        ...
        data_dict.update(img=imgs)
        return data_dict
    
    def Collect3D(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.Collect3D, config=config)
        data_dict = {}
        keys=config['KEYS']
        ...
        for key in meta_keys:
            if key in data_dict:
                val = np.array(data_dict[key])
                if isinstance(data_dict[key], list):
                    data_dict[key] = DC(to_tensor(val), stack=True)
                else:
                    data_dict[key] = DC(to_tensor(val), stack=True, pad_dims=1)

        metas = {}
        for key in meta_lis_keys:
            if key in data_dict:
                metas[key] = data_dict[key]

        data_dict["metas"] = DC(metas, cpu_only=True)
        return data_dict
