...
def sample_augmentation(final_dim, resize_lim, bot_pct_lim, rot_lim, rand_flip, is_train, data_dict):
...
def img_transform(
    final_dim, resize_lim, bot_pct_lim, rot_lim, rand_flip, is_train, img, rotation, translation, resize, resize_dims, crop, flip, rotate
):
    # adjust image
...
def set_epoch(epoch):
    ...
    fixed_prob=1.0
    if not fixed_prob:
        set_prob(epoch, max_epoch)

def set_prob(epoch, max_epoch):
    st_prob=1.0
    epoch=None
